...
void MpiViUtility::saveResults(const std::vector<int> &iStepVector, const std::vector<float> &durationVector, const std::vector<float> &jDiffsMaxNorm, const std::vector<float> &jDiffsL2Norm, const std::vector<float> &jDiffsMSE, const std::vector<long> &maxRSSs, const int comInterval, const std::string &nameSchema) {
    ...
    std::string filenameMeasurements = "../results/" + std::string(GIT_COMMIT_HASH) + "_" + nameSchema + "_" + comIntervallString + "_" + datetime() + fileFormat;
    ...
    for (int iMeasurement = 0; iMeasurement < nMeasurements; ++iMeasurement) {
        std::string line = std::to_string(iStepVector[iMeasurement]) + "," + std::to_string(durationVector[iMeasurement]) + "," + std::to_string(maxRSSs[iMeasurement]) + "," + std::to_string(jDiffsMaxNorm[iMeasurement]) + "," + std::to_string(jDiffsL2Norm[iMeasurement]) + "," + std::to_string(jDiffsMSE[iMeasurement]) + "\n";
